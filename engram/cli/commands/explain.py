...
from engram.analysis.prompts import build_comparison_message, build_single_message
...
def explain_command(
    ...
    if not no_cache and not is_comparison:
        ...
        cached = load_cached_analysis(exp_dir)
    ...
    estimated_cost, est_input, est_output = estimate_analysis_cost(config, user_message)

    if not _confirm_cost(config.model, estimated_cost, est_input, est_output, yes):
        raise typer.Exit(0)

    # Run analysis
    result = call_llm(config, user_message)

    # Cache single-experiment results
    if not is_comparison:
        exp_dir = root / 'experiments' / experiment_a
        save_analysis(exp_dir, result)
