...
def test_save_results_stores_label_when_provided(tmp_path: Path):
    ...
    exp_dir.mkdir(parents=True)
    save_results(exp_dir, 'labeled', 'impl', 'ds', [], label='prompt-v2')
    metadata, _ = load_results(exp_dir)
...
def test_save_results_strips_label_whitespace(tmp_path: Path):
    ...
    exp_dir.mkdir(parents=True)
    save_results(exp_dir, 'trimmed', 'impl', 'ds', [], label='  spaced  ')
    metadata, _ = load_results(exp_dir)
...
def test_save_results_omits_label_when_none(tmp_path: Path):
    ...
    exp_dir.mkdir(parents=True)
    save_results(exp_dir, 'nolabel', 'impl', 'ds', [])
    metadata, _ = load_results(exp_dir)
    assert 'label' not in metadata


def test_save_results_does_not_persist_short_id(tmp_path: Path):
    """Short ids are a display-only view; results.json must not carry one."""
    exp_dir = tmp_path / 'experiments' / 'nosid'
    exp_dir.mkdir(parents=True)
    save_results(exp_dir, 'nosid', 'impl', 'ds', [])
    metadata, _ = load_results(exp_dir)
    assert 'short_id' not in metadata
...
def test_transforms_recorded_in_snapshot(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    exp_id = _run(tmp_path, 'impl', 'small', concurrency=1)
